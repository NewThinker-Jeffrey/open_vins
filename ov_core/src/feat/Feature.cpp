...
void Feature::clean_old_measurements(const std::vector<double> &valid_times) {
  double max_valid_time = *std::max_element(valid_times.begin(), valid_times.end());
  ...
  for (auto const &pair : timestamps) {
    ...
    while (it1 != timestamps[pair.first].end()) {
      if (*it1 < max_valid_time && std::find(valid_times.begin(), valid_times.end(), *it1) == valid_times.end()) {
...
void Feature::clean_future_measurements(double timestamp_now) {
  // Loop through each of the cameras we have
  for (auto const &pair : timestamps) {
    ...
    // Loop through measurement times, remove ones that are newer than the specified one
    while (it1 != timestamps[pair.first].end()) {
      if (*it1 > timestamp_now) {
        it1 = timestamps[pair.first].erase(it1);
        it2 = uvs[pair.first].erase(it2);
        it3 = uvs_norm[pair.first].erase(it3);
      } else {
